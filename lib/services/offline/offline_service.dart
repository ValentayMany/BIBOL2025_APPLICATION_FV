...
class OfflineService {
  ...
  /// 
  /// **Returns:**
  /// - [ConnectivityResult] - ประเภทการเชื่อมต่อ
    ...
    try {
      ...
      if (await isOnline()) {
        try {
          ...
          final newsList = newsResponse.topics
              .map((topic) => {
                    'id': topic.id,
                    'title': topic.title,
          ...
          AppLogger.error('Failed to fetch fresh news', tag: 'OFFLINE', error: e);
    ...
    try {
      ...
      if (await isOnline()) {
        try {
          ...
          final coursesList = coursesResponse.courses
              .map((course) => {
                    'id': course.id,
                    'title': course.title,
                    'icon': course.icon,
